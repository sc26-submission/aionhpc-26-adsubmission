...
DEFAULT_JOB_CSV = Path("ANL-ALCF-DJC-POLARIS_20250101_20251231") / "ANL-ALCF-DJC-POLARIS_20250101_20251231.csv"
DEFAULT_STATUS_CSV = Path("ANL-ALCF-MACHINESTATUS-POLARIS_20250101_20251231") / "ANL-ALCF-MACHINESTATUS-POLARIS_20250101_20251231.csv"
...
def load_jobs(path: Path) -> pd.DataFrame:
    jobs = pd.read_csv(path, low_memory=False)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reconstruct 2025 Polaris unallocated operational capacity."
    )
    parser.add_argument("--jobs", type=Path, default=DEFAULT_JOB_CSV)
    parser.add_argument("--status", type=Path, default=DEFAULT_STATUS_CSV)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("polaris_2025_analysis/results"),
    )
...
def main() -> None:
    ...
    timeline_csv = args.output_dir / "polaris_2025_operational_capacity_timeline.csv"
    outages_csv = args.output_dir / "polaris_2025_full_machine_outages.csv"

    jobs = load_jobs(args.jobs)
    separator("JOB DATA")
    print(f"Usable jobs: {len(jobs):,}")

    outages = prepare_outages(load_status(args.status), jobs)
