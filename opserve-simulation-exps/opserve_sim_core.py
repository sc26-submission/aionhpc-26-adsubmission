#!/usr/bin/env python3
"""Small event-driven simulator used by the OPSERVE paper experiments.
...
* request retry when a transient worker disappears, and
* prompt-length-dependent shared-KV transfer delay.
...
TPOT as a function of active sequence concurrency. Shared-KV transfer uses
published Mooncake cross-node measurements as a function of prompt length.
"""
...
# Published Mooncake cross-node RDMA KV-transfer measurements for Qwen3-8B.
# Source: https://kvcache-ai.github.io/Mooncake/performance/vllm/vllm-v1-pd-performance.html
# Values are prompt tokens -> actual transfer time in seconds. Requests below
# prompt_threshold_tokens incur no inter-worker KV-transfer delay.
DEFAULT_KV_TRANSFER_PROFILE: Tuple[Tuple[int, float], ...] = (
    (2048, 0.00251),
    (4096, 0.00475),
    (8192, 0.00884),
    (16384, 0.01643),
...
def interp_kv_transfer_s(
    input_tokens: int,
    profile: Sequence[tuple[int, float]] = DEFAULT_KV_TRANSFER_PROFILE,
) -> tuple[float, bool]:
    return interpolate_profile(float(input_tokens), profile)
...
class SimConfig:
    ...
    latency_half_life_s: float = 30.0
    # Quantile of current queue waiting times folded into the latency signal.
    # This gives the controller timely feedback when a queue is growing before
    # those requests have completed and contributed full phase-latency samples.
    latency_queue_quantile: float = 0.75
    ...
    # latency_share is the controller used for the paper evaluation. Other modes
    # are retained for sensitivity experiments.
    controller_mode: str = "latency_share"  # demand_share | latency_share | slo_pressure | pressure
    ...
    kv_transfer_profile: Tuple[Tuple[int, float], ...] = DEFAULT_KV_TRANSFER_PROFILE
    ...
    # Optional fixed delays retained for sensitivity experiments. The reported
    # evaluation uses the prompt-length-dependent kv_transfer_profile above.
...
class Simulator:
    ...
    def _kv_transfer_delay(self, req: RequestJob) -> float:
        """Prompt-length-dependent shared-KV transfer delay.

        Requests below the configured threshold incur no inter-worker KV-transfer
        delay. The optional kv_handoff_s value is an additive sensitivity offset.
        """
        if req.input_length < self.cfg.prompt_threshold_tokens:
            return max(0.0, self.cfg.kv_handoff_s)
        delay = interp_kv_transfer_s(
            req.input_length,
            self.cfg.kv_transfer_profile,
        )[0]
        return max(0.0, delay + self.cfg.kv_handoff_s)
    ...
    def _finish_stage(self, now: float, payload: dict) -> None:
        ...
        if req.output_length <= 1:
            ...
            # TTFT ends at prefill completion. Disaggregated requests then pay
            # the prompt-length-dependent shared-KV transfer cost before decode.
            ready_time = now + self._kv_transfer_delay(req)
    ...
    def _initial_role(self, wid: WorkerId, now: float) -> str:
        ...
        if self.policy == "static":
            ...
            if self.cfg.controller_mode == "latency_share":
                target_share = self.last_target_prefill_share
        ...
        for wid in ids:
            ...
            for rid, stage in list(worker.in_flight.items()):
                ...
                if stage.stage == "prefill":
                    ...
                    retry_at = (
                        now
                        + self._kv_transfer_delay(req)
                        + max(0.0, self.cfg.recovery_delay_s)
                    )
    ...
    def _recent_exp_mean(
        self,
        dq: Deque[tuple[float, float]],
        now: float,
        half_life_s: float,
    ) -> Optional[float]:
        """Exponentially time-weight recent observations.

        Samples remain bounded by metric_window_s, but newer observations
        receive more weight. This matches the paper's time-decayed latency
        estimates and avoids a stale high-latency episode dominating the
        controller for the full measurement window.
        """
        self._trim(dq, now)
        if not dq:
            return None
        half_life_s = max(1e-9, float(half_life_s))
        decay = math.log(2.0) / half_life_s
        weighted_sum = 0.0
        weight_sum = 0.0
        for timestamp, value in dq:
            weight = math.exp(-decay * max(0.0, now - timestamp))
            weighted_sum += weight * value
            weight_sum += weight
        return weighted_sum / max(1e-12, weight_sum)
    ...
    def _queue_wait_quantile(
        self,
        queue: Deque[int],
        stage: str,
        now: float,
    ) -> Optional[float]:
        """Current queue-wait lower bound for a phase.

        A request still waiting in a phase queue has not yet produced a
        completed phase-latency observation, but its elapsed queueing time is
        already a lower bound on the latency it will experience. Folding a
        robust quantile of these waits into the control signal reduces delayed
        feedback when a queue grows quickly.
        """
        waits: list[float] = []
        for rid in queue:
            req = self.requests[rid]
            if stage == "prefill":
                enqueued = req.prefill_enqueued
            elif stage == "decode":
                enqueued = req.decode_enqueued
            else:
                raise ValueError(f"unknown stage: {stage}")
            if enqueued is not None:
                waits.append(max(0.0, now - float(enqueued)))
        return self._quantile(waits, self.cfg.latency_queue_quantile)
    ...
    def _latency_share_rebalance(self, now: float) -> None:
        # Use the time-decayed phase-latency estimates described in the paper.
        self._trim(self.prefill_phase_latencies, now)
        self._trim(self.decode_phase_latencies, now)
        if min(
            len(self.prefill_phase_latencies),
            len(self.decode_phase_latencies),
        ) < self.cfg.min_control_samples:
            return

        completed_prefill_latency = self._recent_exp_mean(
            self.prefill_phase_latencies,
            now,
            self.cfg.latency_half_life_s,
        )
        completed_decode_latency = self._recent_exp_mean(
            self.decode_phase_latencies,
            now,
            self.cfg.latency_half_life_s,
        )
        if completed_prefill_latency is None or completed_decode_latency is None:
            return

        # Completed requests provide the normal phase-latency signal.  When a
        # queue is growing, however, those measurements can lag because the
        # waiting requests have not completed yet.  Their current wait is
        # already a lower bound on eventual phase latency, so include a robust
        # queue-age estimate in the signal.
        prefill_queue_age = self._queue_wait_quantile(
            self.prefill_queue, "prefill", now
        )
        decode_queue_age = self._queue_wait_quantile(
            self.decode_queue, "decode", now
        )

        prefill_latency = max(
            completed_prefill_latency,
            0.0 if prefill_queue_age is None else prefill_queue_age,
        )
        decode_latency = max(
            completed_decode_latency,
            0.0 if decode_queue_age is None else decode_queue_age,
        )
        ...
        # A high intrinsic phase latency is not by itself evidence of a
        # capacity shortage. Only add capacity to a phase with queued work.
        if delta > 0 and not self.prefill_queue:
            return
        if delta < 0 and not self.decode_queue:
            return
