...
The default matrix contains:
  ...
  * Models: Llama-3.2-3B, Llama-3.1-8B, and Qwen3-8B
  * Policies: Persistent-Only, Static P/D, and MalleServe
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the MalleServe workload/model/policy matrix."
    )
    ...
    parser.add_argument(
        ...
        default="demand_share",
        help=(
            "Adaptive controller. demand_share is the conservative measured-work "
            "controller; latency_share retains the earlier balancer approximation."
        ),
    )
    ...
    parser.add_argument("--prompt-threshold", type=float, default=2048.0)
...
def workload_definitions(root: Path) -> dict[str, dict[str, Any]]:
    ...
    # The distributed package cannot reconstruct a user-generated synthetic
    # trace. Include it automatically when the file is present.
    return {
        key: value
...
def workload_summary(requests, duration_s: float) -> dict[str, float]:
    count = len(requests)
    return {
        "request_count": count,
        "mean_arrival_rate_rps": count / duration_s,
...
def main() -> None:
    ...
    for workload_key in workload_keys:
        ...
        stats = workload_summary(requests, args.trace_duration)
    ...
    if unfinished_total:
        print(
            ...
            "Increase --drain-timeout before using latency values in the paper."
        )
